<template>
    <div class="container-fluid">
        ...
        <div class="container  p-2 mb-5 mt-5 text-center shadow">
             ...
             <div class="row">
                <div class="col-8">
                    ...
                    <div class="row mb-3 mt-3 ms-5 fs-6 me-5">
                        ...
                        <div class="col-9 text-start">
                            <span v-if="waveReportDetails.strategy === 0"> Desktop</span>
                            <span v-if="waveReportDetails.strategy === 1"> Mobilna</span>
                        </div>
                    </div>
                    <hr class="hrStyle">
                    <div class="row mb-3 mt-3 ms-5 fs-6 me-5">
                    ...
                    <div class="row mb-3 mt-3 ms-5 fs-6 me-5">
                        ...
                        <div class="col-9 text-start">
                            {{ waveReportDetails.waveVersion !== null ? waveReportDetails.waveVersion : '-' }}
                        </div>
                    </div>
                    <hr class="hrStyle">
                    <div class="row mb-3 mt-3 ms-5 fs-6 me-5">
                        ...
                        <div class="col-9 text-start">
                            {{ waveReportDetails.systemVersion !== null ? waveReportDetails.systemVersion : '-' }}
                        </div>
                    </div>
                    <hr class="hrStyle">
                    <div class="row mb-3 mt-3 ms-5 fs-6 me-5">
                ...
                <div class="col-4 mt-5 " style="text-align: -webkit-center;">
                    <Chart :dataPoints="waveElements"></Chart>
                </div>
           </div>
                    ...
                    <template #expand="item">
                        <div style="padding: 15px">
                            ...
                            <div class="pt-4"  v-bind:class="{'collapse': !item.isExpanded }" :id="'collapse-'+item.id">
                                <div class="row" v-for="itemElement in item.waveSubElement" :key="itemElement.id">
           ...
           <div v-if="amountOfWarnings !==0 " class="col-8 m-auto pt-3 pb-4">            
                <h4 >Ostrzeżenia 
                    <i v-if="isCollapsedErrors" data-bs-toggle="collapse" href="#collapseWarningTab" role="button" :aria-expanded="isCollapsedErrors" aria-controls="collapseWarningTab" @click="toggleWarning" class="bi bi-arrow-up-circle-fill"></i> 
                    <i v-if="!isCollapsedErrors" data-bs-toggle="collapse" href="#collapseWarningTab" role="button" :aria-expanded="isCollapsedErrors" aria-controls="collapseWarningTab" @click="toggleWarning" class="bi bi-arrow-down-circle-fill"></i> 
                </h4> 
                <div class="collapse" id="collapseWarningTab">
                    <easy-data-table 
                            :headers="headers" 
                            :items="itemsWarnings" 
                            :rows-items=rowsItem 
                            ...
                            header-text-direction="center" 
                            body-text-direction="center"
                            table-class-name="customize-tableWarning" 
                            :loading=loadingFromDB 
                            :search-field="searchField"
                            :search-value="searchValue"  
                    >
                    <template #expand="item">
                        <div style="padding: 15px">
                            ...
                            <button class="btn btn-sm btn-outline-warning col-2 mt-5" type="button" @click="toggleCollapseWarning(item.id)" >
                                {{ !item.isExpanded  ? 'Pokaż wystąpienia' : 'Zwiń' }}
                            </button>
                            <div class="pt-4"  v-bind:class="{'collapse': !item.isExpanded }" :id="'collapse-'+item.id">
                                <div class="row" v-for="itemElement in item.waveSubElement" :key="itemElement.id">
                                ...
                                <button class="btn btn-sm btn-outline-warning col-2 mt-2" type="button" @click="toggleCollapseWarning(item.id)" >
                                {{ 'Zwiń' }}
                            </button>
                            </div>
                        </div>
...
<script>
    ...
    import WaveReportDetails from '../models/WaveReportDetails'
    import { mapMutations } from 'vuex';
    export default {
        data() {
            return {
                waveReportDetails: new WaveReportDetails(),
                amountOfErrors: 0,
                amountOfWarnings: 0,
                amountOfAudits: 0,
                loadingFromDB: false,
        ...
        methods: {
            async getDetails(){
                try {
                    ...
                    let response = await axios.get(this.$store.state.connectionString + 'Wave?guid=' + this.$route.params.id, {headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}}); 
                    if(response.status == 200){
                        this.waveReportDetails = WaveReportDetails.fromResponse(response.data);
                        this.amountOfErrors = this.waveReportDetails.waveElement.filter(element => element.typeOfResult === 0).length;
                        this.amountOfWarnings = this.waveReportDetails.waveElement.filter(element => element.typeOfResult === 1).length;
                        this.amountOfAudits = this.waveReportDetails.waveElement.filter(element => element.typeOfResult === 2).length;

                        this.waveElements[0].y = this.amountOfErrors;
                        this.waveElements[1].y = this.amountOfWarnings;
                        this.waveElements[2].y = this.amountOfAudits;

                        this.waveReportDetails.waveElement.forEach(element => {
                            if(element.typeOfResult == 0)
                                this.itemsErrors.push(element);
                            else if(element.typeOfResult == 1)
                                this.itemsWarnings.push(element);
                        ...
                        document.title = 'Raport '+ this.waveReportDetails.webName;

                        if(this.waveReportDetails.createdById !== null && this.waveReportDetails.createdById !== '')
                            await this.getUser(this.waveReportDetails.createdById);
            ...
            async getDomainName(){
                try {
                    let response = await axios.get(this.$store.state.connectionString + 'Wave/getDomain?guid=' + this.$route.params.id, {headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}});                     
            ...
            toggleCollapseWarning(itemId) {
                const item = this.itemsWarnings.find(i => i.id === itemId);
                if (item) {
                    item.isExpanded = !item.isExpanded;
                }
            },
            ...
            async repeatReport(){
                ...
                infoToast.show();
                await this.execOneReport(this.waveReportDetails);
                infoToast.hide();
            ...
            async execOneReport(item){
                try {
                    ...
                    if(response.status == 200){
                        ...
                        this.$router.replace('/reportDetailsWave/'+response.data[0].id).then(() => {
                            this.$nextTick(() => {
...
.customize-tableWarning {
    ...
    --easy-table-body-row-font-color: #ffffff;
    --easy-table-body-row-background-color: #7c6637a1;
    --easy-table-body-row-height: 50px;
    --easy-table-body-row-font-size: 14px;
