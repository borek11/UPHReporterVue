    ...
    <div class="container-fluid">
        ...
        <div class="container bg-dark p-2 mb-5 text-center">
                        ...
                        <div class="shadow p-3 mb-5 rounded" v-else>
                            ...
                            >
                                ...
                                <template #item-strategy="item">
                                        <div v-if="item.amountOfErrors != null">
                                            {{ item.strategy === 0 ? 'Desktopowy' : 'Mobilny' }}
                                        </div>
                                        <div v-else> - </div>
                                </template>
                                <template #item-amountOfErrors="item">
                                        <div v-if="item.amountOfErrors != null">
                                            {{item.amountOfErrors}}
                                        </div>
                                        <div v-else> - </div>
                                </template>
                                <template #item-amountOfPassed="item">
                                        <div v-if="item.amountOfPassed != null">
                                            {{item.amountOfPassed}}
                                        </div>
                                        <div v-else> - </div>
                                </template>
                                <template v-if="!isButtonExecDisabled" #item-operation="item">
                                        <div class="operation-wrapper" v-if="item.amountOfErrors != null">
                ...
                <h4 class="pt-3">Właściwości raportu</h4>
                    <form class="needs-validation " novalidate> 
                        ...
                        <div class="mt-3 ms-5 me-5 col-3 " id="divRadio">
                            ...
                            <div class="form-floating mt-2 col-form-label-sm">
                                <input type="text" class="form-control" id="VersionForm" v-model="payload.VersionWave" placeholder="Wersja Wave">
                                <label for="VersionForm">Wersja Wave</label>
                            </div>
                            <div class="form-floating mt-2 col-form-label-sm">
                                <input type="text" class="form-control" id="VersionSystemForm" v-model="payload.VersionSystem" placeholder="Wersja systemu">
                                <label for="VersionSystemForm">Wersja systemu</label>
                            </div>
                        </div>
           ...
           <div class="toast-container position-fixed bottom-0 end-0 p-3 ">
            ...
            <div class="toast-container position-fixed bottom-0 end-0 p-3 ">
                <div id="infoTast3" class="toast text-bg-danger" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="true">
                    <div class="toast-header">
                        <strong class="me-auto">Brak</strong>
                        <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                    </div>
                    <div class="toast-body">
                        Brak kluczy Wave do wykonania raportu
                    </div>
                </div>
            </div>
                ...
                >
                    ...
                    <template v-if="!isButtonExecDisabled" #item-operation="item">
                            <div class="operation-wrapper">
                                <i @click="detailsItem(item)" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Wyświetl szczegóły" class="bi bi-info-square-fill iconStyleDetails iconStyle me-2"></i>
                            </div>
                    </template>
                </easy-data-table>
...
<script>
    ...
    import LinksAndReport from '../models/LinksAndReportWave'
    ...
    export default {
        data() {
            return {
                ...
                headersLink: [
                    ...
                    { text: "Zaliczone", value: "amountOfPassed", sortable: true},
                    { text: "Operacje", value: "operation" },
                ],
        ...
        methods: {
            ...
            async getReports(domainName){
                try {
                    let response = await axios.get(this.$store.state.connectionString + 'wave/multiple?domain='+domainName, {headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}});                    
                    this.reports = response.data.map(item => WaveReportOne.fromResponse(item));
            ...
            async getLinksAboutDomain(domainName, generateForExists, strategy){
                try {
                    ...
                    let response = await axios.get(this.$store.state.connectionString + 'wave/linksAndReports?domain='+domainName+'&strategy='+strategy, {headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}});                    
                    
                    if(generateForExists == true){
                        this.webNames = response.data.map(item => item.webName);                    
                    }
                    else {
                        this.webNames = response.data.filter(item => item.reportId === null).map(item => item.webName);                  
                    }
                } catch (error) {
                    console.log(error);
                }
            ...
            async execReports(webName){
                ...
                let infoTastWaveKey = new Toast(document.getElementById('infoTast3'));
                infoTast.show();    
            ...
            async execOneReport(webName){
                try {
                    ...
                    let response = await axios.post(this.$store.state.connectionString + 'wave/report', this.payload, {headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}});                    
                    if(response.status === 200){
                        //console.log("Raport wykonany")
                        this.resultReports.push(response.data);
                        let tempReport = WaveReportDetails.fromResponse(response.data[0])
                        tempReport.amountOfErrors = tempReport.waveElement.filter(element => element.typeOfResult === 0).length;
                        tempReport.amountOfAudits = tempReport.waveElement.filter(element => element.typeOfResult === 2).length;
                        this.waveReportDetails.push(tempReport);
                    }
            ...
            detailsItemFromLinksTable(item){
                ...
                this.$router.push('/reportDetailsWave/'+item.reportId);
            },
        ...
        mounted() {
            this.getDomains();
            document.title = 'Wykonaj raport Wave';
